...
export class Perfil implements OnInit {
  ...
  // Mapa inverso para las mini barras (aún disponible por si se reutiliza)
  private letraACat: { letra: string; campo: keyof Historial }[] = [
    { letra: 'R', campo: 'puntaje_realista' },
    { letra: 'I', campo: 'puntaje_investigador' },
    { letra: 'A', campo: 'puntaje_artistico' },
    { letra: 'S', campo: 'puntaje_social' },
    { letra: 'E', campo: 'puntaje_emprendedor' },
    { letra: 'C', campo: 'puntaje_convencional' },
  ];
  ...
  ngOnInit(): void {
    this.usuarioServicio.obtenerPerfil().pipe(
      tap((data) => { this.usuario = data; this.cd.detectChanges(); }),
  ...
  onInformacionTab(): void { this.active = 'informacion'; }
  onHistorialTab(): void   { this.active = 'historial'; }

  volver(): void { this.location.back(); }
  irATest(): void { this.router.navigate(['/preguntas']); }
  ...
  onSubmit(): void {
    ...
    this.usuarioServicio.actualizarUsuario(this.usuario).pipe(
      tap(() => {
        Swal.fire('Guardado', 'Tu información se actualizó correctamente.', 'success');
  ...
  abrirDetalle(his: Historial): void {
    this.historialSeleccionado = his;
    this.cargarResultados();
  }

  getCarrerasArray(carreras: string): string[] {
    if (!carreras) return [];
    return carreras.split(',').map(c => c.trim()).filter(c => c.length > 0);
  }

  getPuntajesArray(his: Historial): { letra: string; valor: number }[] {
    return this.letraACat.map(({ letra, campo }) => ({
      letra,
      valor: (his[campo] as number) ?? 0,
    }));
  }

  // ── Resultados del radar (modal) ──────────────────────────────────
  ...
  cargarResultados(): void {
    try {
      ...
      this.todosPuntajes = [
        { categoria: 'Realista',     puntaje: h.puntaje_realista      ?? 0, puntajeMaximo: 50 },
        { categoria: 'Investigador', puntaje: h.puntaje_investigador  ?? 0, puntajeMaximo: 50 },
        { categoria: 'Artístico',    puntaje: h.puntaje_artistico     ?? 0, puntajeMaximo: 50 },
        { categoria: 'Social',       puntaje: h.puntaje_social        ?? 0, puntajeMaximo: 50 },
        { categoria: 'Emprendedor',  puntaje: h.puntaje_emprendedor   ?? 0, puntajeMaximo: 50 },
        { categoria: 'Convencional', puntaje: h.puntaje_convencional  ?? 0, puntajeMaximo: 50 },
      ].sort((a, b) => b.puntaje - a.puntaje);

      const puntajeLimite = this.todosPuntajes[2]?.puntaje ?? 0;
      ...
      console.error('Error al cargar resultados del historial:', error);
      this.isLoading = false;
    }
  }
  ...
  getPorcentaje(item: { puntaje: number; puntajeMaximo: number }): number {
